...
package com.masteragb.artestapp;
  ...
  public void onSurfaceCreated(SampleRender render) {
    ...
    try {
      ...
      Texture virtualObjectPbrTexture =
          Texture.createFromAsset(
              render,
              "models/Mushroom/Metallic.jpg",
              Texture.WrapMode.CLAMP_TO_EDGE,
              Texture.ColorFormat.LINEAR);

      //"models/Mushroom/MushroomShiitake.jpg",
      //"models/Mushroom/10192_MushroomShitake_v1-L3.obj"


      //models/88-flowerpot/FlowerPot/FlowerPot.obj - HUGE
      virtualObjectAlbedoTexture =
              Texture.createFromAsset(
                      ...
                      Texture.WrapMode.CLAMP_TO_EDGE,
                      Texture.ColorFormat.SRGB);
      virtualObjectMesh = Mesh.createFromAsset(render, "models/Mushroom/Mush.obj");
...
// Set the rotation angle to 90 degrees (in radians)
      float rotationAngleZ = (float) Math.toRadians(90);

// Generate a random scaling factor between 0.02 and 0.1
      float minScale = 0.02f;
      float maxScale = 0.02f;
      float randomScale = minScale + (maxScale - minScale) * new Random().nextFloat();
...
// Apply the scaling to the model matrix
      Matrix.scaleM(modelMatrix, 0, randomScale, randomScale, randomScale);

// Apply the rotation around the Z-axis to the model matrix
      Matrix.rotateM(modelMatrix, 0, rotationAngleZ, 1, 1, 1);

// Multiply the model matrix by the view matrix (modelViewMatrix) and then by the projection matrix (modelViewProjectionMatrix)
